...
public class Health {
	...
	public static ConfigurationSection health;
	...
	static {
		isEnabled = ConfigManager.getHealthConfig().getBoolean("enabled");
		if(isEnabled) {
			plugin = new RPGStatus();
			health = ConfigManager.getHealthConfig();
			priority = health.getString("priority");
			anchor = health.getString("anchor");
			size_lock = health.getBoolean("size_lock");
			width_rate = health.getDouble("width_rate");
			height_rate = health.getDouble("height_rate");
			shiftX_rate = health.getDouble("shiftX_rate");
			shiftY_rate = health.getDouble("shiftY_rate");
			common_url = health.getString("common_url");
			detail_url = new String[101];
			isUseValueText = health.getBoolean("use_value_text");
			if(isUseValueText) {
				text_priority = health.getString("text_priority");
				text_anchor = health.getString("text_anchor");
				text_scale = (float) health.getDouble("text_scale");
				text_color = health.getString("text_color");
				text_shiftX_rate = health.getDouble("text_shiftX_rate");
				text_shiftY_rate = health.getDouble("text_shiftY_rate");
	...
	//체력 위젯 이미지를 preLoginCache에 추가(로그인 하기 전에 다운로드 하도록 함).
		...
		if(isEnabled) {
			...
			for(i = 0 ; i <= 100 ; i++) {
				if(i == 0) {
					...
					System.out.println(common_url + detail_url[i]);
				}
				if(i != 0) {
	...
	public static void setTextureURL() {
		int i = 0;
		for(i = 100 ; i >= 0 ; i--) {
			String detail = health.getConfigurationSection("details").getString(i + "%");
			if(detail != null) {
				detail_url[i] = detail;
			}
			if(detail == null) {
	...
	public static void searchDown(int i, int repeat) {
		int j = i - 1;
		String detail = health.getConfigurationSection("details").getString(j + "%");
		if(detail != null) {
	...
	//유저 처음 접속 시 체력 위젯 생성 메서드.
	public void create(Plugin plugin, SpoutPlayer player, int maxHealth, int health) {
		if(isEnabled) {
			if(health > maxHealth)
				health = maxHealth;
			...
			int hPercent = (int) ((double) health/maxHealth*100);
			...
			texture.setUrl(common_url + detail_url[hPercent]);
			...
			if(isUseValueText) {
				...
				label.setText(health + "/" + maxHealth).setTextColor(textColor).setScale(text_scale).setWidth(1).setHeight(1);
	...
	//유저 체력 변경 시 체력 위젯 업데이트 메서드.
	public void change(Plugin plugin, SpoutPlayer player, int maxHealth, int health) {
		if(health > maxHealth)
			health = maxHealth;
		String name = player.getName();
		if(playerWidgets.containsKey(name)) {
			...
			playerWidgets.get(name).setUrl(common_url + detail_url[hPercent]);
			
			player.getMainScreen().updateWidget(playerWidgets.get(name));
		}
		if(playerLabels.containsKey(name)) {
			playerLabels.get(name).setText(health + "/" + maxHealth);
