...
public class Mana {
	...
	public static ConfigurationSection mana;
	...
	static {
		isEnabled = ConfigManager.getManaConfig().getBoolean("enabled");
		if(isEnabled) {
			plugin = new RPGStatus();
			mana = ConfigManager.getManaConfig();
			priority = mana.getString("priority");
			anchor = mana.getString("anchor");
			size_lock = mana.getBoolean("size_lock");
			width_rate = mana.getDouble("width_rate");
			height_rate = mana.getDouble("height_rate");
			shiftX_rate = mana.getDouble("shiftX_rate");
			shiftY_rate = mana.getDouble("shiftY_rate");
			common_url = mana.getString("common_url");
			detail_url = new String[101];
			isUseValueText = mana.getBoolean("use_value_text");
			if(isUseValueText) {
				text_priority = mana.getString("text_priority");
				text_anchor = mana.getString("text_anchor");
				text_scale = (float) mana.getDouble("text_scale");
				text_color = mana.getString("text_color");
				text_shiftX_rate = mana.getDouble("text_shiftX_rate");
				text_shiftY_rate = mana.getDouble("text_shiftY_rate");
	...
	//마나 위젯 이미지를 preLoginCache에 추가(로그인 하기 전에 다운로드 하도록 함).
	...
	public static void setTextureURL() {
		int i = 0;
		for(i = 100 ; i >= 0 ; i--) {
			String detail = mana.getConfigurationSection("details").getString(i + "%");
			if(detail != null) {
				detail_url[i] = detail;
			}
			if(detail == null) {
	...
	public static void searchDown(int i, int repeat) {
		int j = i - 1;
		String detail = mana.getConfigurationSection("details").getString(j + "%");
		if(detail != null) {
	...
	//유저 처음 접속 시 마나 위젯 생성 메서드.
	public void create(Plugin plugin, SpoutPlayer player, int maxMana, int mana) {
		if(isEnabled) {
			if(mana > maxMana)
				mana = maxMana;
			...
			int mPercent = (int) ((double) mana/maxMana*100);
			...
			texture.setUrl(common_url + detail_url[mPercent]);
			...
			if(isUseValueText) {
				...
				label.setText(mana + "/" + maxMana).setTextColor(textColor).setScale(text_scale).setWidth(1).setHeight(1);
	...
	//유저 마나 변경 시 체력 위젯 업데이트 메서드.
	public void change(Plugin plugin, SpoutPlayer player, int maxMana, int mana) {
		if(mana > maxMana)
			mana = maxMana;
		...
		if(playerWidgets.containsKey(name)) {
			...
			playerWidgets.get(name).setUrl(common_url + detail_url[mPercent]);
			
			player.getMainScreen().updateWidget(playerWidgets.get(name));
		}
		if(playerLabels.containsKey(name)) {
			playerLabels.get(name).setText(mana + "/" + maxMana);
